    ...
    init(_ vmCreator: VMCreator, _ vm: VirtualMachine) {
        ...
        private func downloadRestoreImage(restoreImage: VZMacOSRestoreImage, completionHandler: @escaping (Error?) -> Void) {
            let downloadTask = URLSession.shared.downloadTask(with: restoreImage.url) { localURL, _, error in
                ...
                if !self.canceled {
                    guard (try? FileManager.default.moveItem(at: localURL!, to: URL(fileURLWithPath: self.vm.path + "/" + VirtualizationFrameworkMacOSSupport.RESTORE_IMAGE_NAME))) != nil else {
                        fatalError("Failed to move downloaded restore image to \(URL(fileURLWithPath: self.vm.path + "/" + VirtualizationFrameworkMacOSSupport.RESTORE_IMAGE_NAME)).")
                    }
            ...
            downloadObserver = downloadTask.progress.observe(\.fractionCompleted, options: [.initial, .new]) { _, change in
